...
/**
 * MOVED HERE FROM app/organization/readiness/page.tsx so the sidebar can
 * reach it.
 *
 * The screen was complete and wired - the first page under app/organization
 * to call Laravel directly - and completely unreachable: it had no row in
 * tblmenumaster_g2g, nothing linked to it, and it could only be opened by
 * typing the URL. Menu 225 was created once to prove the guard and then
 * rolled back, which is why routes/api.php still carries
 * `// menuright:225,view RE-ADD WITH THE MENU`.
 *
 * The content map mounts screens with NO PROPS, so this exports a propless
 * component; the old route stays as a thin wrapper so the URL keeps working
 * for anything that already links to it.
 */
