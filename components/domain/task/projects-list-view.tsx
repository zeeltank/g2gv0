...
export function ProjectsListView() {
  ...
  return (
    <div className="flex h-full flex-col gap-6">
      ...
      <div className="flex flex-col gap-3 rounded-xl border bg-card/40 p-3 sm:flex-row sm:justify-between">
        <div className="relative max-w-md flex-1"><Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><input value={searchInput} onChange={(e) => setSearchInput(e.target.value)} placeholder="Search project, code, or manager…" className="h-10 w-full rounded-lg border bg-background pl-9 pr-3 text-sm" /></div>
        <Select value={status} onChange={(value) => { setStatus(value as ProjectStatus | 'all'); setPage(1) }} options={[{ value: 'all', label: 'All Projects' }, ...options.statuses.filter((v) => v !== 'ARCHIVED').map((value) => ({ value, label: value }))]} className="w-full sm:w-44" />
      </div>
...
function ProjectDrawer({ projectId, open, options, onClose, onChanged }: { projectId: string | null; open: boolean; options: ProjectOptions; onClose: () => void; onChanged: () => void }) {
  ...
  return <Sheet open={open} onOpenChange={(next) => !next && onClose()}><SheetContent className="w-full overflow-y-auto sm:max-w-2xl"><SheetHeader><SheetTitle>{project?.name ?? 'Project'}</SheetTitle><SheetDescription>{project?.code ?? 'Loading project details…'}</SheetDescription></SheetHeader>
    {loading ? <div className="flex h-48 items-center justify-center"><Spinner /></div> : <div className="mt-5 space-y-5">{error && <Notice danger>{error}</Notice>}{message && <Notice>{message}</Notice>}
      ...
      {project && tab === 'tasks' && <div><h3 className="mb-3 font-semibold">Linked tasks</h3><Checklist items={options.tasks.map((t) => ({ id: String(t.id), label: `${t.title} (${t.status ?? 'Pending'})` }))} selected={taskIds} setSelected={setTaskIds} /><Button className="mt-4" onClick={saveTasks}>Save Linked Tasks</Button></div>}
      {project && tab === 'workstreams' && <div className="space-y-4"><div className="grid gap-2 sm:grid-cols-3"><input value={wsName} onChange={(e) => setWsName(e.target.value)} placeholder="Workstream name" className="h-10 rounded-lg border px-3 text-sm" /><Select value={wsOwner} onChange={setWsOwner} options={options.users.map((u) => ({ value: String(u.id), label: u.name }))} placeholder="Owner" /><Select value={wsStatus} onChange={(v) => setWsStatus(v as ProjectStatus)} options={options.statuses.filter((s) => s !== 'ARCHIVED').map((s) => ({ value: s, label: s }))} /></div><div className="flex gap-2"><Button onClick={saveWorkstream} disabled={!wsName.trim()}>{editingWorkstream ? 'Save Workstream' : 'Add Workstream'}</Button>{editingWorkstream && <Button variant="outline" onClick={() => { setEditingWorkstream(null); setWsName(''); setWsOwner(''); setWsStatus('PLANNING') }}>Cancel</Button>}</div><div className="space-y-2">{project.workstreams?.map((item) => <div key={item.id} className="flex items-center justify-between rounded-xl border p-4"><div><p className="font-medium">{item.name}</p><p className="text-xs text-muted-foreground">{item.owner_name || 'No owner'} · {item.status}</p></div><div className="flex gap-2"><Button variant="outline" size="sm" onClick={() => { setEditingWorkstream(item); setWsName(item.name); setWsOwner(item.owner_id ? String(item.owner_id) : ''); setWsStatus(item.status) }}>Edit</Button><Button variant="outline" size="sm" onClick={() => removeWorkstream(item)}>Delete</Button></div></div>)}{!project.workstreams?.length && <p className="rounded-xl border border-dashed p-8 text-center text-sm text-muted-foreground">No workstreams yet.</p>}</div></div>}
...
function Info({ label, value }: { label: string; value: string }) { return <div className="rounded-xl border p-3"><p className="text-xs text-muted-foreground">{label}</p><p className="font-medium">{value}</p></div> }
...
function Status({ status }: { status: ProjectStatus }) { return <span className={cn('w-fit rounded-full border px-2 py-1 text-xs font-semibold', status === 'COMPLETED' ? 'text-success' : status === 'AT RISK' ? 'text-danger' : 'text-primary')}>{status}</span> }
