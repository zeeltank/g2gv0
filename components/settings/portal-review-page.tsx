...
export function PortalReviewPage() {
  ...
  const cards: ReviewCardData[] = [
    ...
    },
    {
      ...
      icon: Network,
      tone: 'primary',
      route: '/module/m1/org-setup/dept-management',
    ...
    },
    {
      ...
      icon: Users,
      tone: 'primary',
      route: '/module/m1/user-management/employee-directory',
    ...
    },
    {
      ...
      icon: ShieldCheck,
      tone: 'warning',
      route: '/module/m1/user-management/role-permissions',
      action: 'View Roles',
  ...
  function downloadSummary() {
    ...
    link.href = url
    link.download = 'portal-review-summary.txt'
    link.click()
  ...
  return (
    <ProtectedLayout>
      <SetupWizardLayout currentStep={5} steps={STEPS} completedSteps={COMPLETED}>
        <div className="mx-auto w-full max-w-[1500px] space-y-5 pb-6">
          <Card>
            <CardContent className="p-5">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div>
                  <h1 className="text-2xl font-bold tracking-tight">Portal Review</h1>
                  <p className="mt-1 text-sm text-muted-foreground">Review all the configurations before you go live.</p>
                </div>
                <Button variant="outline" size="lg" onClick={downloadSummary}>
                  <Download />
                  Download Summary
                </Button>
              </div>
              <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-[2fr_repeat(4,1fr)]">
                <div className="rounded-lg border border-border p-4">
                  ...
                  <Progress className="mt-3" value={readiness} />
                </div>
            ...
            <CardContent className="p-5">
              ...
              {loadError && (
                <Alert variant="destructive" className="mt-6">
                  <CircleAlert />
                  <span>
                    Could not load the latest onboarding data. <button className="font-semibold underline" onClick={loadSummary}>Try again</button>
                  </span>
                </Alert>
              )}
              ...
              <div className="mt-5 grid gap-4 xl:grid-cols-[1fr_1fr_0.7fr]">
                <Card className="shadow-none">
                  ...
                  <CardContent className="space-y-3 px-5 pb-5">
                    ...
                    {summary.employees && summary.employees.warnings > 0 && (
                      <div className="flex items-center justify-between gap-3 text-sm">
                        <span className="flex items-center gap-2">
                          <CircleAlert className="size-4 text-warning" />
                          {summary.employees.warnings} employee{summary.employees.warnings > 1 ? 's have' : ' has'} warnings
                        </span>
                        <Badge variant={checked.warnings ? 'success' : 'warning'}>{checked.warnings ? 'Resolved' : 'Warning'}</Badge>
                      </div>
                    )}
                    <p className="text-xs text-muted-foreground">
                      {Number(Boolean(summary.organization)) + Number(summary.selectedModules.length > 0) + Number(summary.departments.length > 0) + Number(Boolean(summary.employees && summary.employees.total > 0)) + Number(Boolean(summary.roles && summary.roles.roles.length > 0)) + (summary.employees && summary.employees.warnings > 0 ? (checked.warnings ? 1 : 0) : 1)}/6 validations complete
                    </p>
                  </CardContent>
                </Card>
                ...
                <Card className="shadow-none">
                  ...
                  <CardContent className="space-y-2 px-5 pb-5">
                    <Action
                      icon={Users}
                      title={summary.employees && summary.employees.warnings > 0 ? `Review ${summary.employees.warnings} Employee Warning${summary.employees.warnings > 1 ? 's' : ''}` : 'Review Employee Records'}
                      description={summary.employees && summary.employees.warnings > 0 ? 'Some employee records need your attention.' : 'All employee records imported successfully.'}
                      done={checked.warnings}
                      onClick={() => setChecked((x) => ({ ...x, warnings: true }))}
                    />
                ...
                <Card className="border-success/30 bg-success/5 shadow-none">
                  <CardContent className="flex h-full flex-col items-center justify-center p-6 text-center">
                    ...
                    <Button className="mt-5 w-full bg-success hover:bg-success/90" size="lg" onClick={() => setOpen(true)}>
                      <Rocket />
                      Go Live
                    </Button>
              ...
              {saved && (
                <Alert variant="success" className="mt-5">
                  <CheckCircle2 />
                  <span>Your portal review draft has been saved locally.</span>
                </Alert>
              )}
              <div className="mt-5 flex flex-col-reverse gap-3 border-t border-border pt-5 sm:flex-row sm:justify-between">
                <Button variant="outline" size="lg" onClick={() => router.push('/organization/setup')}>
                  <ArrowLeft />
                  Back to Employee Import
                </Button>
        ...
        <AlertDialog open={open} onOpenChange={setOpen}>
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>Make your portal live?</AlertDialogTitle>
              <AlertDialogDescription>
                This finishes setup and opens the HRMS portal to your users. Settings remain editable afterward.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              <Button variant="outline" onClick={() => setOpen(false)}>Cancel</Button>
              <Button className="bg-success hover:bg-success/90" onClick={goLive}>
                <Rocket />
                Confirm Go Live
              </Button>
            </AlertDialogFooter>
      ...
      {STEPS.map((step, i) => {
        ...
        return (
          <li className="relative text-center" key={step.id}>
            ...
            <span className={cn('relative mx-auto flex size-8 items-center justify-center rounded-full border text-sm font-semibold', done && 'border-success bg-success text-success-foreground', current && 'border-primary bg-primary/10 text-primary ring-4 ring-primary/10', !done && !current && 'border-border text-muted-foreground')}>
              {done ? <Check className="size-4" /> : i + 1}
            </span>
            <p className="relative mt-2 text-xs font-semibold">{step.label}</p>
            <p className={cn('mt-1 text-xs', done ? 'text-success' : current ? 'text-primary' : 'text-muted-foreground')}>
...
function ReviewCard({ title, icon: I, tone, action, details, onClick }: ReviewCardData & { onClick: () => void }) {
  return (
    <Card className="flex min-h-72 flex-col overflow-hidden shadow-none">
      <div className="flex items-center gap-3 border-b border-border bg-muted/30 p-4">
        <span className={cn('flex size-10 items-center justify-center rounded-lg', tone === 'success' ? 'bg-success/10 text-success' : tone === 'warning' ? 'bg-warning/10 text-warning' : 'bg-primary/10 text-primary')}>
          <I className="size-5" />
        </span>
        <div>
      ...
      <CardContent className="flex flex-1 flex-col p-4">
        <dl className="space-y-3">
          {details.map(([label, value]) => (
            <div className="border-b border-border pb-2 last:border-0" key={label}>
              <dt className="text-xs text-muted-foreground">{label}</dt>
              <dd className={cn('text-xs font-semibold', label === 'Warnings' && 'text-warning', label === 'Errors' && 'text-destructive', label === 'Successfully Imported' && 'text-success')}>{value}</dd>
            </div>
          ))}
        </dl>
        <Button variant="outline" className="mt-auto w-full border-primary/40 text-primary" onClick={onClick}>
          {action}
        </Button>
      </CardContent>
    </Card>
...
function Readiness({ score, onClick }: { score: number; onClick: () => void }) {
  return (
    <Card className="min-h-72 border-success/20 bg-success/5 shadow-none">
      <CardContent className="flex h-full flex-col items-center p-4 text-center">
        <span className="flex size-10 items-center justify-center rounded-lg bg-success/10 text-success"><Rocket className="size-5" /></span>
        <h2 className="mt-2 text-sm font-semibold">Portal Readiness</h2>
        <div className="mt-4 flex size-28 items-center justify-center rounded-full border-[9px] border-success/20 border-r-success border-t-success">
          <div>
            <p className="text-3xl font-bold">{score}%</p>
            <p className="text-[10px] text-muted-foreground">Readiness Score</p>
          </div>
        </div>
        <p className="mt-4 text-xs font-medium text-success">Your portal is almost ready to go live!</p>
        <Button variant="outline" className="mt-auto w-full border-primary/40 text-primary" onClick={onClick}>
          View Summary
        </Button>
      </CardContent>
...
function CheckRow({ label, done }: { label: string; done: boolean }) {
  return (
    <div className="flex items-center justify-between gap-3 text-sm">
      <span className="flex items-center gap-2">
        {done ? <CheckCircle2 className="size-4 text-success" /> : <CircleAlert className="size-4 text-warning" />}
        {label}
      </span>
...
function Action({ icon: I, title, description, done, onClick }: { icon: Icon; title: string; description: string; done: boolean; onClick: () => void }) {
  return (
    <div className="flex items-center gap-3 rounded-lg border border-border bg-muted/30 p-3">
      <span className={cn('flex size-9 shrink-0 items-center justify-center rounded-lg', done ? 'bg-success/10 text-success' : 'bg-primary/10 text-primary')}>
        <I className="size-4" />
      </span>
      ...
      <Button size="sm" variant="outline" disabled={done} onClick={onClick}>
        {done ? 'Done' : 'Review'}
      </Button>
    </div>
  )
